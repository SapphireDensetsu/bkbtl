...
void Settings_ParseKeyValue(const char* pSectName, const char* pKey, const char* pValue)
{
    //TODO
}
...
void Settings_ParseSettings(char* pText)
{
    ...
    for (;;)  // Text reading loop - line by line
    {
        ...
        *pText++ = 0;  // End of the key
        ...
        if (*pText == 0 || *pText == '\n' || *pText == '\r')
        {
            Settings_ParseKeyValue(pSection, pKey, NULL);
            continue;
        }
        char* pValue = pText;
        ...
        Settings_ParseKeyValue(pSection, pKey, pValue);
